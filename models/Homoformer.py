...
class BasicUniEncoderBlock(nn.Module):
    def __init__(self, dim, input_resolution, num_heads, win_size=8,
        ...
        self.Pmodule = PModule(
            dim=dim, hidden_dim=mlp_hidden_dim, act_layer=act_layer, drop=drop)
    ...
    def forward(self, x, img_size=(128, 128)):
        ...
        x_windows = window_partition(shuffle_x_, self.win_size)
...
class HomoFormer(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
        ...
        self.decoderlayer_3 = BasicUniEncoderLayer(dim=embed_dim * 2,
    ...
    def flops(self):
        ...
        flops += self.encoderlayer_1.flops() + self.dowsample_1.flops(self.reso //
                                                                      2, self.reso // 2)
...
if __name__ == "__main__":
    ...
    xm = torch.ones((1, 1, 512, 512)).cuda()
    model_restoration = model_restoration.cuda()
    total_time = 0
    with torch.no_grad():
        for i in range(NUM):
            start_time = time.time()
            model_restoration(input)
