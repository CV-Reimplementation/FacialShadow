...
class SADC(nn.Module):
    def __init__(self, channels = 64):
        super(SADC, self).__init__()
    ...
    def forward(self, x, mask):
        ...
        ##Stage0
        out0_1, _, _ = self.block0_1(out, mask)
        out0_2, _, _ = self.block0_2(out0_1, mask)
        
        agg0_rgb = self.aggreation0_rgb(torch.cat((out0_1, out0_2), dim = 1))

        ##Stage1
        out1_1, _, _ = self.block1_1(agg0_rgb, mask)
        out1_2, _, _ = self.block1_2(out1_1, mask)
        
        agg1_rgb = self.aggreation1_rgb(torch.cat((agg0_rgb, out1_1, out1_2), dim = 1))

        ##Stage2
        out2_1, _, _ = self.block2_1(agg1_rgb, mask)
        out2_2, _, _ = self.block2_2(out2_1, mask)
        
        agg2_rgb = self.aggreation2_rgb(torch.cat((agg1_rgb, out2_1, out2_2), dim = 1))
        
        ##Stage3
        out3_1, _, _ = self.block3_1(agg2_rgb, mask)
        out3_2, _, _ = self.block3_2(out3_1, mask)
